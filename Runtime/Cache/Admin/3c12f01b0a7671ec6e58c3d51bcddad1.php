...
<?php if(is_array($list)): $n = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><h2 class="sub-header"><?php echo ($vo["title"]); ?><a id="<?php echo ($vo["id"]); ?>" onclick="editInfo(<?php echo ($vo["id"]); ?>)" class="btn btn-link">修改</a></h2>
  ...
  <input type="radio" name="<?php echo ($s["uid"]); ?>" id="inlineRadio1" value="option1"> 录用
</label>
                        <label class="radio-inline">
  <input type="radio" name="<?php echo ($s["uid"]); ?>" id="inlineRadio2" value="option2"> 辞退
</label>
                    </td>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
            </tbody>
            <button type="button" class="btn pull-right btn-danger">提交</button>
        </table>
    </div>
